 ...
 * of RFC 959. <br/>
 * FTPReply存储了一组用于FTP应答代码的常量。为了解释代码的含义，假定熟悉RFC 959。助记常量名称是RFC 959的代码描述的转录。
 * <p>
 * TODO replace with an enum 要用enum代替
 ***/
...
public final class FTPReply
{
     ...
     * server.<br/>
	 *
	 *确定回复代码是否是一个积极的初步响应。从1开始的所有代码都是积极的初步响应。用Postitive的初步回答来表示试探性的成功。在从服务器接收到后续响应之前，不会向FTP服务器发出其他命令。
     *
     * @param reply  The reply code to test.回复测试代码。
     * @return True if a reply code is a postive preliminary response, false
     *         if not.如果回复代码是一个不稳定的初始响应，则为false。
     ***/
	 ...
	 * 确定回复代码是否是一个正完成响应。从2开始的所有代码都是正完成响应。FTP服务器将在命令的最后成功完成时发送一个正完成响应。
     *
     * @param reply  The reply code to test. 测试的回复代码。
     * @return True if a reply code is a postive completion response, false
     *         if not. 如果回复代码是一个难以处理的完成响应，则为false。
     ***/
     ...
     * ready for the PASS command.<br/>
	 * 确定回复代码是否是一个积极的中间响应。从3开始的所有代码都是正的中间响应。FTP服务器将发送一个积极的中间响应，成功完成一个多部分命令序列的一部分。例如，在成功的用户命令之后，将发送一个正的中间响应，以表明服务器已准备好接受PASS命令。
     *
     * @param reply  The reply code to test.回复测试代码。
     * @return True if a reply code is a postive intermediate response, false如果回复代码是一个难以控制的中间响应，则为false。
     *         if not.
     ***/
     ...
     * failure of a command that can be reattempted with success.<br/>
	 * 确定回复代码是否是一个负的瞬态响应。从4开始的所有代码都是负的瞬态响应。FTP服务器将会对一个命令的失败发送一个负面的临时响应，可以重新尝试成功。
     *
     * @param reply  The reply code to test.回复测试代码。
     * @return True if a reply code is a negative transient response, false
     *         if not.如果回复代码是一个负的瞬态响应，则为false。
     ***/
     ...
     * failure of a command that cannot be reattempted with success.<br/>
	 确定回复代码是否是一个消极的永久响应。从5开始的所有代码都是负面的永久性响应。FTP服务器将对不能重新尝试成功的命令的失败发送一个负面的永久响应。
     *
     * @param reply  The reply code to test.回复测试代码。
     * @return True if a reply code is a negative permanent response, false
     *         if not.如果回复代码是一个否定的永久响应，则为false。
     ***/
    ...
    /**
     * Determine if a reply code is a protected response.确定回复代码是否为受保护的响应。
     * @param reply  The reply code to test.回复测试代码。
     * @return True if a reply code is a protected response, false
     *         if not.如果回复代码是受保护的响应，则为false。
     * @since 3.0
     */
